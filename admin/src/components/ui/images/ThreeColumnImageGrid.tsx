...
const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (!product_id) {
    ...
    reader.onload = () => {
      const newImage = { 
        filename: file.name, 
        fileBuffer: reader.result as string, // ตรวจสอบว่า reader.result เป็น base64 string ที่ถูกต้อง
      };
      setTempImages((prev) => [...prev, newImage]);
      onImagesUpdate([...images, ...tempImages, newImage]);
    };
    reader.readAsDataURL(file); // อ่านไฟล์เป็น base64 string
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      const newImage = { 
        ...
        fileBuffer: `data:image/png;base64,${data.fileBuffer}`, // ตรวจสอบว่า fileBuffer เป็น base64 string ที่ถูกต้อง
      };
      setImages([...images, newImage]);
      onImagesUpdate([...images, ...tempImages, newImage]);
    } else {
