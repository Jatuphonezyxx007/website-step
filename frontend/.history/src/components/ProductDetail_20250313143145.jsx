...
const ProductDetail = () => {
  const { id } = useParams();
  const [product, setProduct] = useState(null);
  const [mainImage, setMainImage] = useState(""); // เก็บภาพหลัก
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/products/${id}`)
      ...
      .then((data) => {
        ...
        setMainImage(`/products${data.product.images_main}`); // ตั้งค่าภาพหลักเริ่มต้น
      })
      .catch((error) => console.error("Error fetching product:", error));
  }, [id]);
  ...
  return (
    <div className="product-detail">
      <div className="image-container">
        {/* ภาพหลัก */}
        <img className="main-image" src={mainImage} alt={product.product_name} />

        {/* ภาพรอง (Thumbnail) */}
        <div className="thumbnail-container">
          <img
            className="thumbnail"
            src={`/products${product.images_main}`}
            alt="Main"
            onClick={() => setMainImage(`/products${product.images_main}`)}
          />
          {product.supplementary_images &&
            product.supplementary_images.map((img, index) => (
              <img
                ...
                onClick={() => setMainImage(`/products${img}`)}
              />
            ))}
        </div>
      </div>
