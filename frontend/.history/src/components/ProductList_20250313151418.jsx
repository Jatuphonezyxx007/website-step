...
const ProductsList = () => {
  ...
  const [currentPage, setCurrentPage] = useState(1);
  const totalPages = 5; // สมมุติว่าเรามี 5 หน้า

  useEffect(() => {
    // ดึงข้อมูลสินค้า (สามารถใช้ API จริงได้)
    fetch("http://localhost:3000/api/products")
      .then((res) => res.json())
      .then((data) => setDisplayedProducts(data.products || []))
  ...
  return (
    <div className="product-list-container">
      ...
      {/* Sidebar (Filters) */}
      <div className={`filter-sidebar ${isSidebarOpen ? "open" : "hidden"} md:block`}>
        <h3 className="text-xl font-bold mb-4">Filter Products</h3>
        <form>
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">หมวดหมู่สินค้า</label>
            <CheckboxGroup value={selectedCategories} onChange={handleCategoryChange}>
              {["Electronics", "Clothing", "Accessories"].map((category, index) => (
                <Checkbox key={index} value={category}>{category}</Checkbox>
              ))}
            </CheckboxGroup>
          </div>
      ...
      <div className="product-grid">
        ...
        <div className="grid-container">
          {displayedProducts.length > 0 ? (
            displayedProducts.map((product, index) => (
              <Card key={index} isPressable className="product-card">
                <CardBody className="overflow-hidden p-0">
                  <Image
                    isZoomed
                    alt={product.product_name}
