...
const ProductList = () => {
  ...
  <CheckboxGroup value={selectedCategories} onChange={handleCategoryChange}>
    {categories.map((category, index) => (
      ...
      {/* ปุ่มเปิด/ปิดฟิลเตอร์ (โชว์เฉพาะจอเล็ก) */}
      <button
        onClick={toggleFilter}
        className="lg:hidden fixed top-4 left-4 z-50 bg-blue-600 text-white px-4 py-2 rounded-md shadow-md"
      >
        {isFilterVisible ? "ปิดตัวกรอง" : "เปิดตัวกรอง"}
      </button>

      {/* Sidebar ฟิลเตอร์ */}
      <div
        className={`filter-sidebar fixed top-0 left-0 h-full w-72 bg-white shadow-lg p-4 overflow-auto transition-transform duration-300 ease-in-out
        ${isFilterVisible ? "translate-x-0" : "-translate-x-full"} lg:relative lg:translate-x-0 lg:w-1/4`}
      >
        <h3 className="text-xl font-bold mb-4">Filter Products</h3>
        <form>
