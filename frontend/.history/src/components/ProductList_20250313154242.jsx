    ...
    {categories.map((category, index) => (
      <Checkbox key={`category-${index}-${category.value}`} value={category.value.toString()}>
        {category.label}
          ...
          <div className={`filter-sidebar sticky top-0 h-screen overflow-auto bg-white shadow-lg p-4 ${isOpen ? "w-64" : "w-16"} transition-all duration-300`}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="block w-full text-left p-2 bg-blue-500 text-white rounded-md mb-4 focus:outline-none"
      >
      ...
      {isOpen && (
        <form>
          ...
          <button type="button" className="apply-filters-btn bg-green-500 text-white p-2 rounded-md w-full">
            Apply Filters
          </button>
        </form>
      )}
