...
{/* ปุ่มแสดง Sidebar บนจอเล็ก */}
<button
  className="md:hidden bg-blue-500 text-white px-4 py-2 rounded-lg w-full mb-4"
...
<div className="flex flex-col md:flex-row gap-6">
  ...
  <div
    className={`fixed inset-0 bg-white shadow-lg w-3/4 md:w-1/4 p-6 overflow-auto transform ${
      showFilter ? "translate-x-0" : "-translate-x-full"
    } transition-transform duration-300 ease-in-out md:static md:translate-x-0`}
  >
    {/* ปุ่มปิด Sidebar บนจอเล็ก */}
    <button
      className="md:hidden absolute top-4 right-4 text-gray-700 text-lg"
      onClick={() => setShowFilter(false)}
    >
      ✕
    </button>

    <h3 className="text-xl font-bold mb-4">ตัวกรองสินค้า</h3>
    <form>
      {/* หมวดหมู่สินค้า */}
      <div className="mb-4">
        <label className="block text-sm font-medium mb-2">หมวดหมู่สินค้า</label>
      ...
      {/* ช่วงราคา */}
      <div className="mb-4">
        <label className="block text-sm font-medium mb-2">ช่วงราคา</label>
        <div className="flex gap-2">
          <input type="number" className="w-1/2 border rounded px-3 py-2" placeholder="ต่ำสุด" />
          <input type="number" className="w-1/2 border rounded px-3 py-2" placeholder="สูงสุด" />
        </div>
      </div>
      ...
      <button type="button" className="w-full bg-blue-500 text-white py-2 rounded-lg">
        ใช้ตัวกรอง
      </button>
    </form>
  </div>
              ...
              <div className="mt-2">
                <p
                  className={`text-xs font-bold px-3 py-1 rounded-full ${
                    product.status === "in_stock" ? "bg-green-500 text-white" : "bg-red-500 text-white"
                  }`}
                >
                  {product.status === "in_stock" ? "มีสินค้า" : "สินค้าหมด"}
                </p>
              </div>
            </CardFooter>
          </Card>
