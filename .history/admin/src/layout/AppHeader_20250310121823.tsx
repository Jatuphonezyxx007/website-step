...
//                   <li key={product.product_id} className="px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800">
//                     <Link to={`/product/${product.product_id}`} className="block text-gray-700 dark:text-white">
...
interface AppHeaderProps {
  onSearch: (query: string) => void; // ✅ ฟังก์ชันส่งค่าการค้นหาไป Home.tsx
}
...
const AppHeader: React.FC<AppHeaderProps> = ({ onSearch }) => {
  ...
  useEffect(() => {
    onSearch(searchTerm); // ✅ ส่งค่าการค้นหาไป Home.tsx ทันทีที่พิมพ์
  }, [searchTerm, onSearch]);
  ...
  return (
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="flex flex-col items-center justify-between flex-grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          <button className="items-center justify-center w-10 h-10 text-gray-500 border-gray-200 rounded-lg dark:border-gray-800 lg:flex dark:text-gray-400 lg:h-11 lg:w-11 lg:border" onClick={handleToggle}>
            {/* Icon Sidebar */}
          </button>
