...
export default function SignInForm({ onLogin = () => {} }) {
  ...
  // ✅ ตรวจสอบว่าเคยเข้าสู่ระบบแล้วหรือยัง (หลังจากรีเฟรชหน้า)
  useEffect(() => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) {
      navigate("/dashboard"); // ✅ เปลี่ยนเส้นทางไปที่ Dashboard อัตโนมัติถ้าเข้าสู่ระบบแล้ว
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        localStorage.setItem("user", JSON.stringify(data.user)); // ✅ บันทึกข้อมูลผู้ใช้
        onLogin(data.user); // ✅ แจ้ง component อื่นว่ามีการเข้าสู่ระบบ
