...
export default function ThreeColumnImageGrid({ onImagesUpdate = () => {} }) {
  const { product_id } = useParams(); // ดึง product_id จาก URL
  const [images, setImages] = useState<{ filename: string; fileBuffer?: string; isMain?: boolean; existing?: boolean }[]>([]);
  const [loading, setLoading] = useState(false);
    ...
    try {
      ...
      if (data.success) {
        ...
        updatedImages[index] = { 
          ...
          fileBuffer: `data:image/png;base64,${data.fileBuffer}`, 
          isMain: images[index].isMain, 
          existing: true 
        };
        setImages(updatedImages);
        onImagesUpdate(updatedImages);
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!product_id) {
      const reader = new FileReader();
      reader.onload = () => {
        const newImage = { filename: file.name, fileBuffer: reader.result as string, isMain: false };
        const updatedImages = [...images, newImage];
        setImages(updatedImages);
        onImagesUpdate(updatedImages);
      };
