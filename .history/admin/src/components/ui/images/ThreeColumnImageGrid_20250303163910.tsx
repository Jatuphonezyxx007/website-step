...
export default function ThreeColumnImageGrid({ productId, initialImages }: ThreeColumnImageGridProps) {
  ...
  const handleDelete = async (index: number) => {
    try {
      ...
      const data = await response.json();
      if (data.success) {
        // โหลดข้อมูลภาพใหม่หลังจากลบ
        const updatedImagesResponse = await fetch(`http://localhost:3000/api/images/${productId}`);
  ...
  const handleFileUpload = async (file: File, index: number) => {
    ...
    try {
      ...
      if (data.success) {
        setImages((prev) =>
          prev.map((img, i) =>
            i === index ? `${data.path}?timestamp=${new Date().getTime()}` : img
          ).concat(index >= prev.length ? [`${data.path}?timestamp=${new Date().getTime()}`] : [])
        );        
      } else {
        alert("❌ อัปโหลดรูปไม่สำเร็จ: " + data.message);
      }
