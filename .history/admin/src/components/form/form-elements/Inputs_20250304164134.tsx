...
interface InputsProps {
  productId: number;
  onChange: (updatedData: { product_name: string; detail: string; category_id: number }) => void;
}

const Inputs: React.FC<InputsProps> = ({ productId, onChange }) => {
  const { product, categories, loading } = useProductData(productId);
  ...
  // Handle Change และส่งค่ากลับไปให้ Parent Component (EditForm.tsx)
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const newFormData = { ...formData, [e.target.name]: e.target.value };
    setFormData(newFormData);
    onChange(newFormData); // ส่งค่าที่อัปเดตกลับไปให้ `EditForm.tsx`
  };
  ...
  return (
    <div>
      <ComponentCard title="แก้ไขข้อมูลสินค้า">
        <DefaultInputs name="product_name" label="ชื่อสินค้า" value={formData.product_name} onChange={handleChange} />
        <TextArea name="detail" label="รายละเอียดสินค้า" value={formData.detail} onChange={handleChange} />
        <SelectInputs 
          name="category_id"
