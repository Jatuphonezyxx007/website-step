...
function Model({ url, onLoad, onError }: { url: string; onLoad: () => void; onError: () => void }) {
  ...
  try {
    gltf = useGLTF(url);
    console.log("✅ โหลดโมเดลสำเร็จ:", gltf);
  } catch (error) {
    console.error("❌ โหลดโมเดลล้มเหลว:", error);
    onError();
    return <Html center><p className="text-red-500">❌ โหลดโมเดลล้มเหลว</p></Html>;
  }

  if (!gltf || !gltf.scene) {
    console.error("🚨 โมเดลไม่ถูกต้อง หรือไฟล์เสีย:", url);
    onError();
    return <Html center><p className="text-red-500">🚨 โมเดลไม่ถูกต้อง หรือไฟล์เสีย</p></Html>;
  }
...
export default function Dropzone3D() {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    if (acceptedFiles.length > 0) {
      ...
      const url = URL.createObjectURL(file);
      console.log("📌 สร้าง URL สำเร็จ:", url);
      setModelUrl(url);
      setBlobUrl(url);
