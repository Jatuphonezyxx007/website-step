...
export interface DefaultInputsProps {
  initialValues?: {
    productName?: string;
    email?: string;
    categoryId?: string; // ใช้สำหรับเก็บค่า category จากฐานข้อมูล
  };
...
export default function DefaultInputs({
  ...
  const [productName, setProductName] = useState(initialValues.productName || "");
  const [email, setEmail] = useState(initialValues.email || "");
  // ตั้งค่าเริ่มต้นสำหรับ selectedCategory ด้วย initialValues.categoryId
  ...
  // หาก categories ที่ส่งเข้ามาไม่ว่าง ให้ใช้ค่าที่ส่งเข้ามา แต่ถ้าว่างจะไม่ใช้ fallback (ตามที่ต้องการ)
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <Select
            ...
            onChange={handleSelectChange}
            // ส่งค่า selectedCategory เพื่อ pre-select option จากฐานข้อมูล
