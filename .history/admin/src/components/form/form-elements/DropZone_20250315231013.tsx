...
export default function Dropzone3D({ product_id, onFileSelected }: { product_id: string; onFileSelected: (file: File) => void }) {
  ...
  return (
    <ComponentCard title="3D Viewer">
      <motion.div
        {...getRootProps()}
        className={`border border-dashed rounded-xl p-7 lg:p-10 cursor-pointer ${
          isDragActive ? "border-blue-500 bg-gray-100 dark:bg-gray-800" : "border-gray-300 bg-gray-50 dark:border-gray-700 dark:bg-gray-900"
        }`}
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
      >
        ...
        {modelUrl ? (
          ...
          <motion.div className="flex flex-col items-center">
            <p className="text-gray-500 text-lg">{isDragActive ? "📂 วางไฟล์ 3D model ที่นี่" : "📁 ลากหรือคลิกเพื่อเลือกไฟล์ 3D (.glb, .gltf)"}</p>
          </motion.div>
        )}
      </motion.div>
    </ComponentCard>
