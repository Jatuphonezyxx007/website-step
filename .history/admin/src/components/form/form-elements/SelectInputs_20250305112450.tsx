...
export default function SelectInputs() {
  ...
  const [selectedCategory, setSelectedCategory] = useState(""); // ค่าหมวดหมู่ที่เลือก
  ...
  useEffect(() => {
    const fetchProductCategory = async () => {
      ...
      try {
        ...
        if (data.success) {
          setProductCategory(String(data.product.category_id)); // แปลงเป็น string ให้ตรงกับ Select
        }
      } catch (error) {
  ...
  // 🛠 ตั้งค่า default เมื่อ `categories` และ `productCategory` โหลดเสร็จ
  useEffect(() => {
