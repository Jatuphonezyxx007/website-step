...
export default function SelectInputs() {
  ...
  const [productCategory, setProductCategory] = useState(""); // เก็บค่า category_id ของสินค้านั้นๆ
  ...
  useEffect(() => {
    const fetchProductCategory = async () => {
      ...
      try {
        ...
        if (data.success) {
          setProductCategory(String(data.product.category_id)); // แปลงเป็น string ให้ตรงกับค่าใน <Select>
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (categories.length > 0 && productCategory) {
      console.log("✅ Setting default category:", productCategory);
      ...
      <div>
        ...
        <div className="relative">
          <Select
            ...
            value={selectedCategory || productCategory} // ตั้งค่า default เป็น `category_id` ของสินค้า
            onChange={handleSelectChange}
            className="dark:bg-dark-900"
          />
        </div>
