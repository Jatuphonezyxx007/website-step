      ...
      if (tempImages.some((img) => img.isEdited)) {
        for (const img of tempImages) {
          if (img.isEdited) {
            ...
            formData.append("product_id", product_id);
            formData.append("old_filename", img.filename);
            formData.append("image_index", tempImages.indexOf(img));
            const response = await fetch(`http://localhost:3000/api/update-image`, {
              method: "POST",
              body: formData,
            });
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <br />
          <div className="flex items-center justify-end gap-5">
            {/* <Button type="button" size="sm" variant="danger" onClick={handleDeleteProduct}>
              ลบสินค้า
            </Button> */}
            <Button
            type="button"
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-lg">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">ยืนยันการลบ</h2>
            <p className="text-sm text-gray-600 mb-6">คุณแน่ใจหรือไม่ว่าต้องการลบสินค้านี้?</p>
