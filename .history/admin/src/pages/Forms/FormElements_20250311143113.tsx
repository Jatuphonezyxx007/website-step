...
export default function ProductForm() {
  ...
  useEffect(() => {
    if (!product_id) return; // ✅ หยุดการโหลดข้อมูลถ้าเป็นสินค้าใหม่
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const formData = {
      ...
      category_id: selectedCategory,  // ตรวจสอบให้แน่ใจว่ามีค่า
    };
    ...
    try {
      ...
      if (product_id) {
        ...
        response = await fetch("http://localhost:3000/api/add-product", {  // ✅ ตรวจสอบว่า API ใช้ POST และ path ถูกต้อง
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData),
        });
      ...
      navigate("/dashboard");  // กลับไปหน้า dashboard หลังเพิ่มสำเร็จ
    } catch (error) {
