...
import { Card, CardBody, CardFooter, Image, Pagination, Input } from "@nextui-org/react";
import { CheckboxGroup, Checkbox } from "@nextui-org/react";
...
export default function ProductList() {
  ...
  return (
    <div className="product-list-container">
      <h1 className="text-2xl font-bold mb-4">หมวดหมู่ : {category.label}</h1>
      <Input
        type="text"
      ...
      <CheckboxGroup value={selectedCategories} onChange={setSelectedCategories}>
        <Checkbox value={category.value.toString()}>{category.label}</Checkbox>
      </CheckboxGroup>

      <div className="grid-container">
        {displayedProducts.length > 0 ? (
          displayedProducts.map((product, index) => (
            <Card key={index} isPressable shadow="sm" onPress={() => navigate(`/product-detail/${product.id}`)}>
              <CardBody>
                <Image
                  isZoomed
                  alt={product.name}
