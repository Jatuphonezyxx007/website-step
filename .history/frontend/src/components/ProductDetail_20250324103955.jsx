...
// // ตั้งค่า Modal สำหรับแอป (ควรทำครั้งเดียวในแอป)
// Modal.setAppElement("#root"); // หรือ element ที่เป็น root ของแอปคุณ
...
//       return () => {
//         zoomImage.removeEventListener("mousemove", handleMouseMove);
//         zoomImage.removeEventListener("mouseleave", handleMouseLeave);
//       };
//     }
//   }, [isModalOpen]); // ใช้ isModalOpen เป็น dependency
...
//           </motion.div>
//         ) : (
//           <p className="text-gray-500 text-center">🚫 No 3D model available for this product.</p>
//         )}
//       </div>
//             {/* Modal */}
//             <Modal
...
const ProductDetail = () => {
  ...
  const [selectedImage, setSelectedImage] = useState(""); // เพิ่ม state สำหรับรูปภาพที่เลือก
          ...
          >
            <SwiperSlide>
              <img
                ...
                src={`http://localhost:3000/products/${product.main_image}`}
                alt={product.product_name}
                onClick={() => openModal(`http://localhost:3000/products/${product.main_image}`)}
              />
            </SwiperSlide>
            {product.supplementary_images &&
              product.supplementary_images.map((img, index) => (
                <SwiperSlide key={index}>
                  <img
                    ...
                    src={`http://localhost:3000/products/${img}`}
                    alt={`Supplementary ${index}`}
                    onClick={() => openModal(`http://localhost:3000/products/${img}`)}
                  />
                </SwiperSlide>
              ))}
          </Swiper>
