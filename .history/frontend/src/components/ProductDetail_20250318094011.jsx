...
const ProductDetail = () => {
  ...
  const images = [product.main_image, ...(product.supplementary_images || [])];
  ...
  return (
    <div className="container mx-auto px-4 py-8 pt-[200px]">
            ...
            {images.map((img, index) => (
              <SwiperSlide key={index}>
                <img
                  ...
                  src={`http://localhost:3000/products/${img}`}
                  alt={index === 0 ? product.product_name : `Supplementary ${index}`}
                  onClick={() => {
                    setIsOpen(true);
                    setPhotoIndex(index);
                  }}
            ...
            {images.map((img, index) => (
              <SwiperSlide key={index}>
                <img
                  className="w-20 h-20 cursor-pointer border rounded-lg hover:border-gray-500"
                  src={`http://localhost:3000/products/${img}`}
                  alt={index === 0 ? "Main Thumbnail" : `Thumbnail ${index}`}
                  onClick={() => {
                    setIsOpen(true);
                    setPhotoIndex(index);
                  }}
      ...
      {isOpen && (
        <Lightbox
          mainSrc={`http://localhost:3000/products/${images[photoIndex]}`}
          nextSrc={`http://localhost:3000/products/${images[(photoIndex + 1) % images.length]}`}
          prevSrc={`http://localhost:3000/products/${images[(photoIndex + images.length - 1) % images.length]}`}
