...
export default function Navbar() {
  ...
  <DropdownTrigger>
    <Button variant="bordered" startContent={<Squares2X2Icon className="w-5 h-5" />}>
      หมวดหมู่
    </Button>
  </DropdownTrigger>
    ...
    {categories.length > 0 ? (
      categories.map((category, index) => (
        <DropdownItem
          ...
          textValue={category.name} // ✅ เพิ่ม textValue เพื่อแก้ปัญหา accessibility
          onPress={() => navigate(`/categories/${category.id}`)} // ✅ เปลี่ยนจาก onClick เป็น onPress
        >
          {category.name}
        </DropdownItem>
      ))
    ) : (
      <DropdownItem disabled textValue="ไม่มีหมวดหมู่สินค้า">ไม่มีหมวดหมู่สินค้า</DropdownItem>
    )}
  </DropdownMenu>
</Dropdown>
