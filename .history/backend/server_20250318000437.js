...
app.post("/api/save-images", upload.array("images"), async (req, res) => {
  try {
    ...
    // ดึงจำนวนภาพที่มีอยู่แล้วในฐานข้อมูล
    const [imageCount] = await pool.query(
      "SELECT COUNT(*) AS count FROM product_images WHERE product_id = ?",
      [product_id]
    );
    const nextIndex = imageCount[0].count + 1; // ตั้งชื่อไฟล์ใหม่โดยใช้ index ถัดไป
    ...
    for (let file of images) {
      ...
      await pool.query(
        ...
        [product_id, filename, nextIndex === 1 ? 1 : 0] // ตั้งค่า is_main เป็น 1 สำหรับภาพแรกเท่านั้น
      );

      nextIndex++; // เพิ่ม index สำหรับภาพถัดไป
    }
