...
app.put('/api/products/:product_id', async (req, res) => {
  ...
  console.log("📌 รับค่า images_main:", images_main); // ตรวจสอบค่า images_main

  let imageName = null;
  if (images_main && images_main.includes('.')) {
    const fileExtension = images_main.split('.').pop().split('?')[0];
    console.log("🔍 นามสกุลไฟล์ที่ได้รับ:", fileExtension); // ตรวจสอบนามสกุล

    imageName = `/${product_id}_1.${fileExtension}`;
  } else {
    console.warn("⚠️ images_main ไม่มีนามสกุลหรือเป็นค่า null");
  }
...
app.delete('/api/delete/image/:product_id/:index', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT id, path FROM product_images WHERE product_id = ? ORDER BY id LIMIT 1 OFFSET ?`,
