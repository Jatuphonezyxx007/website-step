...
app.get('/api/products', async (req, res) => {
  try {
    // ✅ ดึง categoryId จาก query parameter และตรวจสอบว่าเป็นตัวเลข
    const categoryId = req.query.category ? parseInt(req.query.category, 10) : null;

    // ❌ หาก categoryId ไม่ถูกต้อง ให้ตอบกลับ 400 Bad Request
    if (!categoryId || isNaN(categoryId)) {
      return res.status(400).json({ success: false, message: "Invalid category ID" });
    }
    ...
    const query = `
      SELECT 
        ...
        category_id
      FROM products 
      WHERE category_id = ?  -- ✅ กรองสินค้าตามหมวดหมู่
      ORDER BY product_id ASC`;
