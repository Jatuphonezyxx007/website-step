...
app.get("/api/products/search", async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT product_id, product_name, category_id, created_at, series_id, product_image_id, path AS main_image FROM products LEFT JOIN product_images ON product_images.product_id = products.product_id AND product_images.is_main = 1 WHERE product_name LIKE ?",
      [`%${searchQuery}%`]
    );
    res.json({ success: true, products: rows });
  } catch (error) {
