...
app.post("/api/save-images", upload.array("images"), async (req, res) => {
  try {
    const { product_id } = req.body;
    const images = req.files; // 🔥 รูปภาพจาก FormData
    ...
    images.forEach((file, index) => {
      const fileExt = path.extname(file.originalname);
      const filename = `product_${product_id}_${index + 1}${fileExt}`;
      const filePath = path.join(savePath, filename);
      fs.writeFileSync(filePath, file.buffer);

      if (index === 0) {
        // ✅ บันทึกเป็นภาพหลัก
        pool.query("UPDATE products SET images_main = ? WHERE product_id = ?", [filename, product_id]);
      } else {
        // ✅ บันทึกเป็นภาพรอง
        pool.query("INSERT INTO product_images (product_id, path) VALUES (?, ?)", [product_id, filename]);
      }
    });
