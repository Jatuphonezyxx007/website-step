...
// ตั้งค่า CORS ให้รองรับ frontend และ admin
const corsOptions = {
  origin: ["http://localhost:3000", "http://localhost:5000", "http://admin.mysite.com", "http://www.mysite.com"],
  credentials: true
};
...
//   }
// });
// API: ดึงข้อมูลสินค้าทั้งหมด หรือเฉพาะหมวดหมู่
app.get('/api/products', async (req, res) => {
  try {
    let query = `
      SELECT 
    ...
    if (req.query.category) {
      ...
      queryParams.push(parseInt(req.query.category, 10));
    }
