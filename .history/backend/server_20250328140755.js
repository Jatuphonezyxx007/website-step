...
const datasetStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, "../admin/public/dataset");
    if (!fs.existsSync(uploadPath)) {
      fs.mkdirSync(uploadPath, { recursive: true });
    }
    cb(null, uploadPath);
  ...
  filename: (req, file, cb) => {
    ...
    if (!product_id) return cb(new Error("Missing product_id"), null);

    // ใช้ product_id ตั้งชื่อชั่วคราวก่อน แล้วค่อยอัปเดตชื่อภายหลังใน handler
    const ext = path.extname(file.originalname);
    cb(null, `temp_${product_id}${ext}`);
  }
});
...
app.post("/api/upload-file", uploadDataset.single("file"), async (req, res) => {
  ...
  if (!req.file || !product_id) {
    return res.status(400).json({ success: false, message: "Missing file or product_id" });
  }
  ...
  try {
    ...
    const productName = rows[0].product_name.replace(/[^a-zA-Z0-9_-]/g, "_");
    const ext = path.extname(req.file.originalname);
    const newFilename = `${productName}${ext}`;
    const newPath = path.join(__dirname, "../admin/public/dataset", newFilename);
    ...
    if (detailRows.length > 0) {
      await pool.query(
        "UPDATE product_details SET datasheet = ?, updated_at = CURRENT_TIMESTAMP WHERE product_id = ?",
    ...
    res.status(200).json({ success: true, filePath: `/dataset/${newFilename}` });
  } catch (error) {
    console.error("Error saving datasheet:", error);
    res.status(500).json({ success: false, message: "Failed to save datasheet" });
  }
});
