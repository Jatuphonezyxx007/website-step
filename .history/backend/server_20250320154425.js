...
const storageDetailImage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const product_id = req.body.product_id; // ตรวจสอบว่า product_id ถูกส่งมาหรือไม่
    const fileExt = path.extname(file.originalname).toLowerCase(); // ตรวจสอบนามสกุลไฟล์
    const newFilename = `${product_id}${fileExt}`; // ตั้งชื่อไฟล์ใหม่
    console.log("New filename:", newFilename); // ตรวจสอบชื่อไฟล์ที่ถูกสร้าง
    cb(null, newFilename);
  },
});
...
app.post("/api/upload-detail-image", uploadDetailImage.single("file"), async (req, res) => {
  try {
    const { product_id } = req.body;
    console.log("Received product_id:", product_id); // ตรวจสอบค่า product_id
    console.log("Received file:", req.file); // ตรวจสอบว่าไฟล์ถูกส่งมาหรือไม่
    ...
    // บันทึกลงในฐานข้อมูล
    const [result] = await pool.query(
      "INSERT INTO product_detail (product_id, detail_image) VALUES (?, ?)",
      [product_id, filePath]
    );
    console.log("Database result:", result); // ตรวจสอบผลลัพธ์จากการบันทึกข้อมูล
