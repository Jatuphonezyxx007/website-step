...
// กำหนดตำแหน่งที่จะเก็บไฟล์ที่อัปโหลดและวิธีการตั้งชื่อไฟล์
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, '../admin/public/products')); // ไฟล์จะถูกเก็บในโฟลเดอร์นี้
  },
  filename: (req, file, cb) => {
    const { product_id, index } = req.params;
    const fileExtension = file.mimetype.split('/')[1]; // ใช้นามสกุลไฟล์จาก mimetype
    cb(null, `${product_id}_${index}.${fileExtension}`); // ตั้งชื่อไฟล์ที่อัปโหลด
  },
});
