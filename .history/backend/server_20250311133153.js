...
app.post("/api/add-product", upload.array("images"), async (req, res) => {
  const { product_name, category_id, series_id, details } = req.body;
  const images = req.files;
  ...
  try {
    ...
    for (let index = 0; index < images.length; index++) {
      ...
      else await connection.query("INSERT INTO product_images (product_id, path) VALUES (?, ?)", [product_id, filename]);
    }

    // อัปเดตภาพหลักของสินค้า
    if (mainImage) {
      await connection.query("UPDATE products SET images_main = ? WHERE product_id = ?", [mainImage, product_id]);
    }
