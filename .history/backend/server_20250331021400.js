...
app.put("/api/products/:product_id", async (req, res) => {
  try {
    const { product_id } = req.params;
    const { product_name, detail, spec, datasheet, category_id } = req.body;
    ...
    // // ✅ เปลี่ยนชื่อไฟล์ datasheet ถ้าชื่อสินค้าเปลี่ยน
    ...
    if (details.length > 0) {
      ...
      await pool.query(
        ...
        [product_id, detail, spec, `${sanitizedNewName}.pdf`] // กรณีเพิ่มใหม่ ตั้งชื่อ datasheet ให้เลย
      );
    }
