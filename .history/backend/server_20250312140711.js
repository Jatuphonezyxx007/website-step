...
app.get('/api/products', async (req, res) => {
  try {
    ...
    console.log(`📌 Received categoryId: ${categoryId}`); // Debug ดูค่าที่ได้รับ
    ...
    const [rows] = await pool.query(query, [categoryId]);
    console.log(`📌 Found ${rows.length} products for category ${categoryId}`); // Debug ว่ามีสินค้าถูกดึงมาได้กี่รายการ

    if (rows.length > 0) {
      res.status(200).json({ success: true, products: rows });
    } else {
      res.status(404).json({ success: false, message: `No products found in category ${categoryId}` });
    }
  } catch (error) {
    console.error("Error fetching products:", error);
    res.status(500).json({ success: false, message: "Server error", error: error.message });
