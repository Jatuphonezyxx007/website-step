...
app.use(
  "/products",
  express.static(path.join(__dirname, "../admin/public/products"))
);
//ให้บริการ static สำหรับร 3d
app.use(
  "/3d",
  express.static(path.join(__dirname, "../admin/public/products_3d"))
);
// ให้บริการไฟล์ static สำหรับรูปผู้ใช้
app.use(
  "/images/user",
  express.static(path.join(__dirname, "../admin/public/images/user"))
);
// ให้บริการไฟล์ static สำหรับรูปรายละเอียดสินค้า
app.use(
  "/products-detail",
  express.static(path.join(__dirname, "../admin/public/products_detail"))
);
// ให้บริการไฟล์ static สำหรับ datasheet
app.use(
  "/datasheet",
  express.static(path.join(__dirname, "../admin/public/datasheet"))
);
...
app.get("/api/product", async (req, res) => {
  try {
    ...
    console.log(
      `📌 Executing SQL Query: ${query} with categoryId = ${categoryId}`
    );
    ...
    res.status(200).json({
      ...
      message:
        rows.length > 0 ? null : `ไม่มีสินค้าสำหรับหมวดหมู่ ${categoryId}`,
    });
  } catch (error) {
    console.error("❌ Error fetching products:", error);
    res
...
app.post("/api/upload-image-temp", upload.single("image"), async (req, res) => {
  try {
    ...
    if (!product_id || product_id === "null") {
      return res
        .status(400)
        .json({ success: false, message: "Product ID is missing or invalid" });
    }
...
app.post("/api/save-images", upload.array("images"), async (req, res) => {
  try {
    ...
    if (!product_id) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid data (missing product_id)" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Images saved successfully" });
  } catch (error) {
...
app.post("/api/upload-3d", upload3d.single("file"), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error uploading 3D file" });
  }
});
...
app.put("/api/products/:product_id", async (req, res) => {
  try {
    const { product_id } = req.params;
    const { product_name, detail, datasheet, category_id } = req.body;
    ...
    if (oldDatasheet) {
      const ext = path.extname(oldDatasheet); // เช่น .pdf
      const oldPath = path.join(
        __dirname,
        "../admin/public/datasheet",
        oldDatasheet
      );
      ...
      // ถ้าไฟล์เก่ายังอยู่ และชื่อเปลี่ยน ➜ เปลี่ยนชื่อ
      if (fs.existsSync(oldPath) && sanitizedOldName !== sanitizedNewName) {
      ...
      // ถ้ามีไฟล์ใหม่เข้ามาผ่าน /upload-file มันจะอัปโหลดไฟล์ใหม่ให้อยู่แล้ว
    }

    // ✅ อัปเดต/เพิ่มรายละเอียดสินค้า
    ...
    if (details.length > 0) {
      await pool.query(
        "UPDATE product_details SET detail = ?, updated_at = CURRENT_TIMESTAMP WHERE product_id = ?",
        [detail, product_id]
      );
    } else {
      await pool.query(
        "INSERT INTO product_details (product_id, detail, datasheet) VALUES (?, ?, ?, ?)",
        [product_id, detail, `${sanitizedNewName}.pdf`] // กรณีเพิ่มใหม่ ตั้งชื่อ datasheet ให้เลย
      );
    }
...
app.get("/api/products/:product_id/3d", async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "3D model not found" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching 3D model" });
  }
});
...
//     ]);
//     await connection.query(
//       "DELETE FROM product_3d_models WHERE product_id = ?",
//       [product_id]
//     );
...
//         // ลบไฟล์ DataSheet
//         const datasheetDir = path.join(__dirname, "../admin/public/datasheet");
//         dataSheet.forEach((datasheet) => {
//           const filePath = path.join(threeDDir, datasheet.path.replace("/", ""));
//           if (fs.existsSync(filePath)) {
//             fs.unlinkSync(filePath);
//             console.log(`🗑️ Deleted 3D model: ${filePath}`);
//           }
...
app.delete("/api/products/:product_id", async (req, res) => {
  ...
  try {
    ...
    // ดึงข้อมูลไฟล์ datasheet ที่เกี่ยวข้องกับสินค้า
    const [dataSheets] = await connection.query(
      "SELECT datasheet FROM product_details WHERE product_id = ? AND datasheet IS NOT NULL",
      [product_id]
    );
    ...
    dataSheets.forEach((item) => {
      if (item.datasheet) {
        const filePath = path.join(
          datasheetDir,
          item.datasheet.replace("/", "")
        );
        if (fs.existsSync(filePath)) {
          fs.unlinkSync(filePath);
          console.log(`🗑️ Deleted datasheet: ${filePath}`);
        }
      }
    });
...
app.post("/api/update-image", upload.single("image"), async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [newFilename, old_filename, product_id]
    );
...
app.delete("/api/delete-image", async (req, res) => {
  ...
  if (!filename || !product_id) {
    return res.status(400).json({
      success: false,
      message: "Filename and product ID are required",
    });
  }
  ...
  try {
    ...
    await connection.query(
      "DELETE FROM product_images WHERE path = ? AND product_id = ?",
      [filename, product_id]
    );
    ...
    if (isMainImage && images.length > 1) {
      ...
      if (nextImage) {
        ...
        const newMainImageFilename = `${product_id}_1${path.extname(
          nextImage.path
        )}`;
        const newMainImagePath = path.join(
          __dirname,
          "../admin/public/products",
...
app.post(
  "/api/admin/add",
  uploadUserImage.single("admin_img"),
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ success: false, message: "กรุณากรอกข้อมูลให้ครบทุกช่อง" });
      }
      ...
      if (admin_phone.length !== 10) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "หมายเลขโทรศัพท์ต้องมี 10 หลัก" });
      }
      ...
      if (existingUser.length > 0) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "ชื่อผู้ใช้งานนี้มีอยู่แล้ว" });
      }
