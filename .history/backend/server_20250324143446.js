...
app.post("/api/upload-3d", upload3d.single("file"), async (req, res) => {
  try {
    ...
    // );
    await connection.execute(
      "REPLACE INTO product_3d_models (product_id, path) VALUES (?, ?)",
      [product_id, filePath]
    );
...
// const storageDetailImage = multer.diskStorage({
//   destination: (req, file, cb) => {
//     const uploadPath = path.join(__dirname, "../admin/public/products_detail");
//     if (!fs.existsSync(uploadPath)) {
//       fs.mkdirSync(uploadPath, { recursive: true });
//     }
//     cb(null, uploadPath);
//   },
//   filename: (req, file, cb) => {
//     const product_id = req.body.product_id; // ตรวจสอบค่า product_id
//     const fileExt = path.extname(file.originalname).toLowerCase(); // นามสกุลไฟล์
//     const newFilename = `${product_id}${fileExt}`; // ตั้งชื่อไฟล์
//     console.log("New filename:", newFilename); // Log ชื่อไฟล์
//     cb(null, newFilename);
//   },
// });

// const uploadDetailImage = multer({ storage: storageDetailImage });

// app.post("/api/upload-detail-image", uploadDetailImage.single("file"), async (req, res) => {
//   try {
//     const { product_id } = req.body;
...
//     // ตรวจสอบว่ามี product_id นี้อยู่ในตารางหรือไม่
//     const [rows] = await pool.query("SELECT * FROM product_details WHERE product_id = ?", [product_id]);

//     if (rows.length > 0) {
//       // อัปเดตรายการที่มีอยู่แล้ว
//       const [updateResult] = await pool.query(
//         "UPDATE product_details SET detail_image = ? WHERE product_id = ?",
//         [filePath, product_id]
//       );
//       console.log("Database update result:", updateResult);
//     } else {
//       // เพิ่มรายการใหม่หากยังไม่มี
//       const [insertResult] = await pool.query(
//         "INSERT INTO product_details (product_id, detail_image) VALUES (?, ?)",
//         [product_id, filePath]
//       );
...
//     res.status(500).json({ success: false, message: "Error uploading detail image" });
//   }
// });
