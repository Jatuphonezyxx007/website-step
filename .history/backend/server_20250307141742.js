...
app.post("/api/admin/login", async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      const validPassword = await argon2.verify(admin.admin_pwd, password);
      if (validPassword) {
        req.session.user = { id: admin.id, username: admin.admin_user, email: admin.admin_email, phone: admin.admin_phone };
        console.log("✅ เข้าสู่ระบบสำเร็จ:", req.session.user);
        return res.status(200).json({ success: true, message: "✅ เข้าสู่ระบบสำเร็จ", user: req.session.user });
      }
    }
    return res.status(401).json({ success: false, message: "❌ ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง" });
  } catch (error) {
