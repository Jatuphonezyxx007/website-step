...
app.post("/api/save-images", upload.array("images"), async (req, res) => {
  try {
    ...
    let imageIndex = existingImages.length + 1; // ถ้ามีภาพอยู่แล้วให้ต่อเลขไปเรื่อยๆ

    for (let index = 0; index < images.length; index++) {
      const file = images[index];
      const fileExt = path.extname(file.originalname);
      const filename = `${product_id}_${imageIndex}${fileExt}`;
      const filePath = path.join(savePath, filename);

      // ✅ ถ้าภาพมีอยู่แล้ว ให้ข้าม
      if (existingImageSet.has(filename)) {
        console.log(`⚠️ Skipping duplicate image: ${filename}`);
        continue;
      }
      ...
      if (imageIndex === 1) {
        ...
        await pool.query("UPDATE products SET images_main = ? WHERE product_id = ?", [filename, product_id]);
        console.log(`📦 Updated main product image: ${filename}`);
      } else {
        // ✅ บันทึกเป็นภาพรอง
        await pool.query("INSERT INTO product_images (product_id, path) VALUES (?, ?)", [product_id, filename]);
