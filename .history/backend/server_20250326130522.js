...
// ใน server.js
app.use(cors({
  origin: 'http://localhost:3000', // หรือ origin ของคุณ
  credentials: true
}));
...
// ในทุก API endpoint ตรวจสอบให้แน่ใจว่าส่งกลับเป็น JSON เสมอ
app.get('/api/admins', async (req, res) => {
  try {
    ...
    // แก้ไขการสร้าง URL รูปภาพ
    const adminsWithImageUrl = rows.map(admin => ({
      ...admin,
      admin_img: admin.admin_img 
        ? `/images/user/${admin.admin_img.replace(/^\//, '')}` // เอา / หน้าสุดท้ายออกถ้ามี
    ...
    // ส่งกลับเป็น JSON แม้แต่เมื่อเกิด error
    res.status(500).json({ 
      success: false, 
