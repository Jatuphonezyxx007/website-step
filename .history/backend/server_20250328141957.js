...
// ใน server.js ก่อนส่วนของ endpoint
...
app.post("/api/upload-file", uploadDataset.single("file"), async (req, res) => {
  ...
  try {
    // ดึงข้อมูลสินค้าเพื่อเอา product_name
    ...
    // สร้างชื่อไฟล์ใหม่จาก product_name
    const originalName = productRows[0].product_name;
    ...
    // ตรวจสอบว่าโฟลเดอร์ datasheet มีหรือไม่
    ...
    if (existingDetail.length > 0 && existingDetail[0].datasheet) {
      const oldFilePath = path.join(__dirname, "../admin/public/datasheet", existingDetail[0].datasheet);
      if (fs.existsSync(oldFilePath)) {
        fs.unlinkSync(oldFilePath);
      }
    }

    // เปลี่ยนชื่อไฟล์และย้ายไปยังตำแหน่งที่ต้องการ
    fs.renameSync(req.file.path, newPath);
    ...
    res.status(500).json({ 
      success: false, 
      message: "Failed to save datasheet",
      error: error.message
    });
  }
});
