...
app.post("/api/products", upload.array("images"), async (req, res) => {
  try {
    const { product_name, category_name, detail, installation_type, screen_size } = req.body;
    ...
    let [category] = await db.promise().query("SELECT * FROM categories WHERE category_name = ?", [category_name]);
    let category_id;

    if (category.length === 0) {
      // 🆕 เพิ่มหมวดหมู่ใหม่หากไม่มี
      const [result] = await db.promise().query(
        "INSERT INTO categories (category_name, category_image) VALUES (?, ?)",
        [category_name, "default.png"]
      );
    ...
    const [productResult] = await db.promise().query(
      ...
      [product_name, category_id, req.files.length > 0 ? req.files[0].filename : null]
    );
    ...
    // 🖼️ อัปโหลดรูปภาพเพิ่มเติม
    if (req.files.length > 1) {
      const imagePaths = req.files.slice(1).map((file) => [product_id, file.filename]);
      await db.promise().query("INSERT INTO product_images (product_id, path) VALUES ?", [imagePaths]);
    }
