...
app.post("/api/upload-image-temp", upload.single("image"), async (req, res) => {
  try {
    ...
    const filename = `product_${product_id}_${index}${fileExt}`;
...
// ✅ API: บันทึกข้อมูลและ copy ไฟล์ไป `/public/products`
app.post("/api/save-images", async (req, res) => {
  try {
    ...
    const savePath = path.join(__dirname, "public", "products");
    if (!fs.existsSync(savePath)) {
      fs.mkdirSync(savePath, { recursive: true });
    }

    images.forEach((img, index) => {
      const base64Data = img.fileBuffer.replace(/^data:image\/\w+;base64,/, "");
      const filePath = path.join(savePath, img.filename);
      fs.writeFileSync(filePath, base64Data, "base64");
      
      if (index === 0) {
        // อัปเดตภาพหลัก (`images_main`) ใน `products`
        pool.query("UPDATE products SET images_main = ? WHERE product_id = ?", [img.filename, product_id]);
      } else {
        // บันทึกภาพรองลง `product_images`
        pool.query("INSERT INTO product_images (product_id, path) VALUES (?, ?)", [product_id, img.filename]);
      }
    });
