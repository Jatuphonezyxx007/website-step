...
// ตั้งค่าอัปโหลดไฟล์
const storage = multer.diskStorage({
  destination: "./public/products", // บันทึกลงโฟลเดอร์ `public/products`
  filename: async (req, file, cb) => {
    const productId = req.body.product_id;
    const fileExt = path.extname(file.originalname);
    
    // 🔍 ดึงลำดับของรูปภาพล่าสุดจากฐานข้อมูล
    ...
    const newFileName = `product_${productId}_${index}${fileExt}`; // ตั้งชื่อไฟล์ตาม index
    cb(null, newFileName);
  },
});
...
app.post("/api/upload-image", upload.single("image"), async (req, res) => {
  try {
    ...
    // 🔍 ตรวจสอบว่าภาพนี้เป็นภาพแรกหรือไม่
    ...
    if (existingImages[0].count === 0) {
      // 🔹 ถ้าเป็นภาพแรก ให้อัปเดต `images_main` ใน `products`
      await pool.query("UPDATE products SET images_main = ? WHERE product_id = ?", [
        filename,
        product_id,
      ]);
    } else {
      // 🔹 ถ้าไม่ใช่ภาพแรก บันทึกลง `product_images`
