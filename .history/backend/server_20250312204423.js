...
app.get('/api/products', async (req, res) => {
  try {
    ...
    // ✅ ตรวจสอบว่า category_id ถูกส่งไปใน Query อย่างถูกต้อง
    const query = `
      SELECT 
        product_id AS id, 
        product_name AS name, 
        ...
        category_id
      FROM products 
      WHERE category_id = ?  -- ✅ เงื่อนไขสำคัญ ต้องกรองสินค้าตามหมวดหมู่
      ORDER BY product_id ASC`;

    const [rows] = await pool.query(query, [categoryId]);

    console.log(`📌 SQL Query Executed: ${query} with [${categoryId}]`);
    console.log(`📌 Found ${rows.length} products for category ${categoryId}`);
